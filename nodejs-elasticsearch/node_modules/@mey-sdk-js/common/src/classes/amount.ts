...
export default class Amount {
    ...
    constructor(value: AmountArg|Buffer|Uint8Array, unit = '', newUnit?: string) {
        if (value instanceof Amount || (<any>value).value) {
            return <Amount>value;
        }
    ...
    div(otherAmount: AmountArg): Amount {
        let newUnit;
        const sum = JSBI.divide(this.value as JSBI, Amount.toJSBI(otherAmount, 'gas'));
    ...
    mul(otherAmount: AmountArg): Amount {
        const sum = JSBI.multiply(this.value as JSBI, Amount.toJSBI(otherAmount, 'gas'));
        return new Amount(sum, this.unit);
    }
}