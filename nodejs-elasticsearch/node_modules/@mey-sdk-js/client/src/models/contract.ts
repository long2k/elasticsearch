import { Address, Amount, constants, fromNumber, base58check } from '@mey-sdk-js/common';
import Tx from './tx';
...
export class FunctionCall {
    ...
    /**
     * Generate transaction object that can be passed to :meth:`meyClient.accounts.sendTrasaction`
     * 
     ...
     *     mey.accounts.sendTransaction(functionCall.asTransaction({
     *         from: myAddress
     *     })).then(result => {
     *         console.log(result);
     *     })
     ...
     * You usually do not need to call this function yourself, :meth:`MeyCoinClient.queryContract` takes care of that.
     * 
     * .. code-block:: javascript
     * 
     *     import { Contract } from '@mey-sdk-js/client';
     *     const contract = Contract.fromAbi(abi).atAddress(address);
     *     const functionCall = contract.someAbiFunction();
     *     mey.queryContract(functionCall).then(result => {
     *         console.log(result);
     *     })
     * 
 ...
 *     mey.queryContractState(query).then(result => {
 *         console.log(result);
 *     })
 */
 ...
 *     mey.queryContract(contract.someAbiFunction()).then(result => {
 *         console.log(result);
 *     })
 * 
 */
